...
function RightHiddenbar() {
  const { shop, showShopSetup, completeShopSetup, isAuthenticated, isSubscriptionCheckDone } = useAuth()
...
const updateDailySales = async (itemsSold, salesAmount) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const element = document.getElementsByClassName("dash-switcher");
      const currentScrollPosition = element.scrollY || element.scrollTop;
  ...
  return (
    <>
   {(isAuthenticated && !isSubscriptionCheckDone) ? <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', fontSize: '1.2rem', color: '#333' }}>{"Vérification de l'état de l'abonnement..."}</div>
      : (
        <div className="dashboard">
          {openShopSetup && <ShopSetupForm close={closeShopSetup} />}
